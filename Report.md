...
The approach of RuHuman is to first evaluate the performance of existing submissions of the ASVSpoof competitions with the ASVSpoof 2021 evaluation dataset with audio samples that have alternative additive sound sources along with the speaker source. Additionally, the custom dataset will have their audio samples from real speakers and synthesized with some of the most popular voice cloning software (e.g. TortoiseTTS).  Afterwards, a user interface will be implemented such that any gateway device that has access to the internet can record/upload their audio sample and get their results through HTTP requests.
